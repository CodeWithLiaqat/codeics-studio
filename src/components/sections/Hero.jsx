...
// CanvasFrame ko Lazy Load kiya taake 3D engine Text Rendering (LCP) ko block na kare
const CanvasFrame = lazy(() => import("../three/CanvasFrame"));
  ...
  return (
    <section aria-labelledby="hero-heading" className="relative isolate overflow-hidden">
      ...
      <div className="mx-auto grid w-full max-w-7xl items-center gap-10 px-4 pb-16 pt-10 sm:px-8 sm:pt-16 lg:grid-cols-[1.1fr_0.9fr] lg:gap-16 lg:pb-28 lg:pt-24">
        ...
        {/* Left Content Column (Renders Instantly) */}
        <div className="flex min-w-0 flex-col gap-6 sm:gap-8">
          ...
          <h1 id="hero-heading" className="font-display text-balance break-words text-3xl font-semibold leading-[1.04] tracking-tight sm:text-5xl lg:text-7xl">
          ...
          <div className="flex flex-wrap items-center gap-x-5 gap-y-2">
            <a href={heroContent.tertiaryCta.href} target="_blank" rel="noopener noreferrer" className="inline-flex min-h-[44px] items-center gap-2 text-sm text-zinc-400 hover:text-accent-soft rounded-md">
            ...
            <p className="text-sm text-zinc-400">
              Led by <Link to="/contact" className="text-zinc-200 hover:text-accent-soft">{siteConfig.founder.name}</Link>
            </p>
          </div>
        </div>

        {/* Right 3D Visual Scene (Loads in background) */}
        <div className="relative">
          ...
          <div role="region" aria-label="3D Interactive Visual Presentation">
            <Suspense fallback={<div className="h-[60vh] sm:h-[70vh] lg:h-[85vh] w-full rounded-2xl border border-stroke bg-obsidian-100" />}>
      ...
      <div className="relative border-y border-stroke bg-obsidian-50/60 backdrop-blur-xl">
        <dl className="mx-auto grid w-full max-w-7xl grid-cols-2 gap-y-2 px-4 sm:px-8 md:grid-cols-4 md:divide-x md:divide-stroke">
          {heroContent.metrics.map((m) => (
            <div key={m.label} className="flex min-w-0 flex-col gap-1 py-5 pr-3 md:px-6 md:py-6 md:first:pl-0 md:last:pr-0">
              <dt className="order-2 text-[11px] leading-snug tracking-wide text-zinc-400 sm:text-xs">{m.label}</dt>
