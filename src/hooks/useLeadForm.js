...
export default function useLeadForm({ source = "website", onSuccess, presetService = "" } = {}) {
  ...
  // Sync if presetService updates externally via URL navigation
  useEffect(() => {
  ...
  const submit = useCallback(
    async (e) => {
      ...
      // 2. Dispatch EmailJS notification to Outlook inbox
      try {
        ...
        // Log internally without breaking UI success state (lead is already preserved in Supabase)
        if (import.meta.env.DEV) {
          console.error("[EmailJS] Notification dispatch failed:", emailErr?.text || emailErr);
        }
      }

      dispatch({
        type: "success",
        message: "Brief received. You will hear from us within 24 hours.",
      });
